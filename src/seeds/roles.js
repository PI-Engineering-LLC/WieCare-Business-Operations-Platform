...
//   // Clean up existing data for a fresh seed
//   await knex('membership_roles').del(); // Delete first due to FK constraints
//   await knex('role_permissions').del();
//   await knex('roles').del();
//   await knex('permissions').del(); // Ensure permissions are also fresh for mapping

//   // --- Re-seed permissions (should be done before roles to get IDs) ---
//   // (Assuming permissions seed handles resource:action format and returns IDs)
//   // You would ideally call your permissions seed script here or have this data available.
//   // For this example, let's assume `permissions.seed` from the next section runs first or this data is known.
...
//     .insert(initialPermissions)
//     .onConflict(['resource', 'action']) // Use the unique constraint
//     .ignore();
...
//   // --- Insert Roles ---
//   const [clientAdmin] = await knex('roles')
//     .insert({ name: 'client_admin', is_system: true }) // is_system for built-in roles
//     .onConflict('name')
//     .ignore()
//     .returning('*');

//   const [generalUser] = await knex('roles')
//     .insert({ name: 'general_user', is_system: true })
//     .onConflict('name')
//     .ignore()
//     .returning('*');

//   const [platformAdmin] = await knex('roles')
//     .insert({ name: 'platform_admin', is_system: true }) // Add platform_admin role
//     .onConflict('name')
//     .ignore()
//     .returning('*');

//   const [superAdmin] = await knex('roles')
//     .insert({ name: 'super_admin', is_system: true }) // Add super_admin role
//     .onConflict('name')
//     .ignore()
//     .returning('*');
...
//   // Platform Admin permissions (example - assume they have broader platform-level management)
//   if (platformAdmin) {
//     rolePermissionsToInsert.push(
//       { role_id: platformAdmin.id, permission_id: permissionMap.get('platform:users.view') },
//       { role_id: platformAdmin.id, permission_id: permissionMap.get('roles:manage') },
...
//     );
//   }
//   // Super Admin typically has all permissions, but can be explicitly listed or implicitly handled in middleware
//   // For seeding, let's give super admin all created permissions
//   if (superAdmin) {
//       permissions.forEach(p => {
//           rolePermissionsToInsert.push({ role_id: superAdmin.id, permission_id: p.id });
//       });
...
//   // Filter out any undefined permission_ids if a mapping failed
//   const validRolePermissions = rolePermissionsToInsert.filter(rp => rp.permission_id);
