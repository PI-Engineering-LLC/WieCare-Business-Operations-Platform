...
router.post('/view-private', requireAuth,loadContext,resolveClientContext,
  ...
  try {
    ...
    if (!(req.user.isInternalAdmin || fileKey.startsWith(`${clientId ? clientId + "/" : '' }public/`) || fileKey.startsWith(`${clientId ? clientId + "/" : '' }private/`) || fileKey.startsWith(`public`) || ( fileKey.includes(`/video/`)&& fileKey.startsWith(`public`)))) {
